...
export const validateNicknameHelper = (str, length) => {
  ...
  const nickname =
    String(str).length >= 2 && nicknameRegex.test(str) && String(str).length <= length;
  return nickname;
};
...
export const comparePrices = (referenceNumber, compareNumber) => {
  ...
  return {
    status,
    per: !isNaN(per) ? `${status === "up" ? "+" : ""}${per.toFixed(2).toLocaleString()}%` : "0.00%",
...
export const getMarketStatus = (market) => {
  ...
  // 음수 양수 상태, 클래스바인딩을 위함
  const status = +open === +last ? "default" : +last - +open < 0 ? "down" : "up";
  const per = ((+last - +open) / +open) * 100;
  return {
    status,
    per: !isNaN(per) && "0.00%" ? `${per.toFixed(2).toLocaleString()}%` : "0.00%",
...
export const getMillion = (number) => {
  const result = (+number / 1000000).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  return `${result}백만`;
};
...
export const orderBookUnits = (number, quote_unit) => {
  function getNumber(text) {
    const number = Number(
      typeof text === "string" && text.includes(",") ? text.replaceAll(",", "") : text
    );
    return number;
  }
...
export const toLocalePercent = (text = "0%") => {
  ...
  const str = text + "";
  const result = Number(str.split(",").join("").replace("%", "")).toLocaleString(undefined, {
